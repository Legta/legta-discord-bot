  ...
  async execute(interaction) {
    ...
    https
      .get(imgUrl, async (response) => {
        ...
        if (
          /* length <= 256 * 1024 &&  /*  Checking if the image size is under or equal to 256kb   */ type.toLowerCase() ===
            "image/jpeg" ||
          ...
          await interaction.guild.emojis
            ...
            .then(
              console.log(
                `Created new emoji with name ${emojiName} and url ${imgUrl}`
              ),
              interaction.reply(`Emoji "${emojiName}" created succesfully :)`)
            )
            .catch(console.error);
        } else {
